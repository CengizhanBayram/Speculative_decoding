...
def bootstrap_ci(data: list, n: int = 10_000, ci: float = 0.95) -> tuple:
...
def run_all_statistical_tests(
    ...
    speculative decoding results vs the greedy baseline.

    Returns a flat dict suitable for JSON serialisation.
    """
    results: dict = {}

    for label, spec_df in [("tr", spec_tr_df), ("en", spec_en_df)]:
        lat_base = baseline_df["latency_ms"].tolist()
